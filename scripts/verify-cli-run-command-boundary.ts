...
assert.match(authority, /const submitted = submitWorkbenchValidationJob\(\{[\s\S]*\.\.\.parsed\.request,/)
assert.match(authority, /\.\.\.\(routed\.mode === 'session_aware' \? \{ sessionId: routed\.sessionId \} : \{\}\),/)
assert.match(authority, /\.\.\.\(sessionRunId && !parsed\.request\.runId \? \{ runId: sessionRunId \} : \{\}\)/)
assert.match(authority, /getWorkbenchValidationJob\(parsed\.validationJobId, sourceId\)/)
