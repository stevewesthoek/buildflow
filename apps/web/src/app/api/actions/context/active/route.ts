...
import { getWorkbenchActiveContext, setWorkbenchActiveContext, unwrapActionError, withActionRouteDiagnostics } from '@/lib/actions/gpt'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getWorkbenchActiveContext(auth.bearerToken)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data = await setWorkbenchActiveContext(body, auth.bearerToken)
